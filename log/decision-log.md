...
2025-05-05: [Phase 5.2] DECISION: Shift from Runtime Dynamic Loading to Static Linking Modularity
- Rationale: The primary goal is achieving a clean separation between a physics-agnostic core and modular physics implementations. While runtime dynamic loading offers flexibility, it introduces significant complexity (cross-platform loading, runtime dependency checks, manifest parsing, build system intricacies) that detracts from the core goal during the complex physics migration phase. A statically linked modular system still achieves the crucial core/physics separation and compile-time flexibility with a simpler, more robust core infrastructure.
- Impact: Remove dynamic loading components (dynamic library loader, manifest parsing, discovery). Implement static module registration. Simplifies core system, focuses efforts on physics migration using the property system, and removes runtime failure modes associated with dynamic linking. Loses the ability to swap modules without recompiling SAGE, but this is acceptable for the current project phase. Refactoring plan primary goal reworded to reflect this focus.
