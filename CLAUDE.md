...
- For the current project status, start by reading `log/README.md` and follow the information trail
...
- Running unit tests: begin with `make clean` to clean out the autogenerated files; run with `make <unit-test> && ./tests/<unit-test>`
- Before finishing your work, stop and ask if we should run `make clean && make unit_tests` to check for unintended issues
