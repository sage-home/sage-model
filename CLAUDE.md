...
- Use lldb for debugging: sign the executable with enhanced debugging entitlements, e.g. `./code-sign.sh sage`; input commands into lldb from a file (always end with "quit"), e.g. `lldb --batch -s debug_commands.txt ./sage`; use absolute paths as lldb can change the working directory
