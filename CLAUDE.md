...
- Start by reading `log/README.md` for instructions on the current project status
- For context, read `docs/README.md` for the right documents to review
- Run sage using `./sage ./input/millennium.par` unless prompted differently
...
- All new unit tests should follow the template `docs/templates/test_template.c`
- Unit tests: begin with `make clean` to clean out the autogenerated files; run with `make <unit-test> && ./tests/<unit-test>`
- When you think you've finished your work, stop and ask if we should run `make clean && make unit_tests` to validate
