...
- Use lldb for debugging: sign the sage executable with enhanced debugging entitlements by running `./codesign-sage.sh`; input commands into lldb from a file (always end with "quit"), e.g. `lldb --batch -s debug_commands.txt ./sage`; use absolute paths for parameter files as lldb changes the working directory
