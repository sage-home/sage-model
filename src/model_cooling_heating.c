...
double do_AGN_heating(double coolingGas, const int centralgal, const double dt, const double x, const double rcool, struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    // Calculate redshift-dependent RadioModeEfficiency
    double radio_eff = get_redshift_dependent_parameter(run_params->RadioModeEfficiency, 
                                                      run_params->RadioModeEfficiency_Alpha, z);
    ...
    if(galaxies[centralgal].HotGas > 0.0) {
        if(run_params->AGNrecipeOn == 2) {
            ...
            AGNrate = (2.5 * M_PI * run_params->G) * (0.375 * 0.6 * x) * galaxies[centralgal].BlackHoleMass * radio_eff;
            ...
            if(galaxies[centralgal].Mvir > 0.0) {
                AGNrate = radio_eff / (run_params->UnitMass_in_g / run_params->UnitTime_in_s * SEC_PER_YEAR / SOLAR_MASS)
