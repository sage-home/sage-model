...
double redshift = run_params->ZZ[galaxies[p].SnapNum];

// Get redshift-scaled parameters
double scaled_sf_efficiency = get_redshift_scaled_sf_efficiency(run_params, redshift);
double scaled_feedback_ejection = get_redshift_scaled_feedback_ejection(run_params, redshift);
...
// Use redshift-scaled star formation efficiency here
strdot = scaled_sf_efficiency * (galaxies[p].ColdGas - cold_crit) / tdyn;
} else {
strdot = 0.0;
}
} else {
...
// determine ejection - Use redshift-scaled feedback ejection efficiency
if(run_params->SupernovaRecipeOn == 1) {
if(galaxies[centralgal].Vvir > 0.0) {
ejected_mass =
(scaled_feedback_ejection * (run_params->EtaSNcode * run_params->EnergySNcode) / (galaxies[centralgal].Vvir * galaxies[centralgal].Vvir) -
