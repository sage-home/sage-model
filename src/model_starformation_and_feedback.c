                                ...
                                struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    double reheated_mass = (run_params->SupernovaRecipeOn == 1) ? fb_reheat * stars : 0.0;
    ...
    if(run_params->SupernovaRecipeOn == 1) {
        if(galaxies[centralgal].Vvir > 0.0) {
            ejected_mass =
                (fb_eject * (run_params->EtaSNcode * run_params->EnergySNcode) / (galaxies[centralgal].Vvir * galaxies[centralgal].Vvir) -
                 fb_reheat) * stars;
        } else {
            ejected_mass = 0.0;
        }
