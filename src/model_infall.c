...
void add_infall_to_cgm(const int gal, double infallingGas, struct GALAXY *galaxies)
{
    ...
    // if the halo has lost mass, subtract CGM metals mass first, then the CGM gas
...
void add_infall_to_hot_only(const int gal, double infallingGas, struct GALAXY *galaxies)
{
    float metallicity;

    // if the halo has lost mass, subtract hot metals mass first, then the hot gas
    if(infallingGas < 0.0 && galaxies[gal].MetalsHotGas > 0.0) {
        metallicity = get_metallicity(galaxies[gal].HotGas, galaxies[gal].MetalsHotGas);
        galaxies[gal].MetalsHotGas += infallingGas*metallicity;
        if(galaxies[gal].MetalsHotGas < 0.0) galaxies[gal].MetalsHotGas = 0.0;
...
void add_infall_to_regime(const int gal, double infallingGas, struct GALAXY *galaxies, const struct params *run_params)
{
    // Increment debug counter
    infall_debug_counter++;
    
    // If CGM toggle is off, use original behavior (all to hot gas with CGM interaction)
    if(run_params->CgmOn == 0) {
        add_infall_to_hot(gal, infallingGas, galaxies);
        return;
    }

    // Convert halo mass to units of 10^13 Msun for comparison with threshold
    // Mvir is in units of 10^10 Msun, so divide by 1000 to get 10^13 Msun
    ...
    if(infall_debug_counter % 50000 == 0) {
        ...
        printf("DEBUG INFALL [gal %ld]: Mvir=%.2e (%.2e x10^13), threshold=%.2e x10^13, regime=%s, infall=%.2e\n",
               infall_debug_counter, galaxies[gal].Mvir, halo_mass_1e13, run_params->CgmMassThreshold, regime, infallingGas);
    }

    // Mass-based regime routing with component elimination
    if(halo_mass_1e13 < run_params->CgmMassThreshold) {
        // Low-mass regime: all infall goes to CGM, NO hot gas allowed
        add_infall_to_cgm(gal, infallingGas, galaxies);
        // Ensure no hot gas exists in low-mass regime
        galaxies[gal].HotGas = 0.0;
        galaxies[gal].MetalsHotGas = 0.0;
    } else {
        // High-mass regime: all infall goes to hot gas, NO CGM allowed
        add_infall_to_hot_only(gal, infallingGas, galaxies);
...
void strip_from_satellite_regime(const int centralgal, const int gal, const double Zcurr, struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    if (run_params->CgmOn) {
        ...
        if (halo_mass_1e13 < run_params->CgmMassThreshold) {
            // CGM regime: strip from CGM gas
            strip_from_satellite_cgm(centralgal, gal, Zcurr, galaxies, run_params);
            cgm_regime_count++;
            ...
            // HotGas regime: strip from HotGas (original behavior)
            strip_from_satellite(centralgal, gal, Zcurr, galaxies, run_params);
            hot_regime_count++;
    ...
    if (debug_count % 50000 == 0) {
        printf("Satellite stripping: Processed %ld satellites - CGM regime: %ld, HotGas regime: %ld\n", 
               debug_count, cgm_regime_count, hot_regime_count);
    }
}

void strip_from_satellite_cgm(const int centralgal, const int gal, const double Zcurr, struct GALAXY *galaxies, const struct params *run_params)
{
    double reionization_modifier;
    ...
    if(strippedGas > 0.0) {
        // In CGM regime: strip from CGM gas instead of HotGas
        ...
        // Remove gas from satellite's CGM
        galaxies[gal].CGMgas -= strippedGas;
        galaxies[gal].MetalsCGMgas -= strippedGasMetals;

        // Add stripped gas to central galaxy - determine central's regime
        double central_mass_1e13 = galaxies[centralgal].Mvir / 1000.0;
        if (central_mass_1e13 < run_params->CgmMassThreshold) {
            // Central is also CGM regime: add to CGM
            galaxies[centralgal].CGMgas += strippedGas;
            galaxies[centralgal].MetalsCGMgas += strippedGas * metallicity;
        } else {
            // Central is HotGas regime: add to HotGas
            galaxies[centralgal].HotGas += strippedGas;
            galaxies[centralgal].MetalsHotGas += strippedGas * metallicity;
        }
    }
