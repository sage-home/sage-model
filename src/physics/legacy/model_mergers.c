...
double estimate_merging_time(const int sat_halo, const int mother_halo, const int ngal, struct halo_data *halos, struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    if(SatelliteMass > 0.0 && coulomb > 0.0 && halos[sat_halo].Len >= MinNumPartSatHalo) {
        mergtime = 2.0 *
            1.17 * SatelliteRadius * SatelliteRadius * get_virial_velocity(mother_halo, halos, run_params) / (coulomb * run_params->cosmology.G * SatelliteMass);
    } else {
        mergtime = -1.0;
    }
                             ...
                             struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    if(run_params->physics.AGNrecipeOn) {
...
void grow_black_hole(const int merger_centralgal, const double mass_ratio, struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    if(galaxies[merger_centralgal].ColdGas > 0.0) {
        BHaccrete = run_params->physics.BlackHoleGrowthRate * mass_ratio /
...
void quasar_mode_wind(const int gal, const double BHaccrete, struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    const double quasar_energy = run_params->physics.QuasarModeEfficiency * 0.1 * BHaccrete * 
                               (C / run_params->units.UnitVelocity_in_cm_per_s) * 
                               (C / run_params->units.UnitVelocity_in_cm_per_s);
                                  ...
                                  struct GALAXY *galaxies, const struct params *run_params)
{
    ...
    if(run_params->physics.SupernovaRecipeOn == 1) {
        reheated_mass = run_params->physics.FeedbackReheatingEpsilon * stars;
    } else {
        reheated_mass = 0.0;
    }
    ...
    if(run_params->physics.SupernovaRecipeOn == 1) {
        if(galaxies[centralgal].Vvir > 0.0) {
            ejected_mass =
                (run_params->physics.FeedbackEjectionEfficiency * 
                 (run_params->physics.EtaSNcode * run_params->physics.EnergySNcode) / 
                 (galaxies[centralgal].Vvir * galaxies[centralgal].Vvir) -
                 run_params->physics.FeedbackReheatingEpsilon) * stars;
        } else {
            ejected_mass = 0.0;
        }
    ...
    metallicity = get_metallicity(galaxies[merger_centralgal].ColdGas, galaxies[merger_centralgal].MetalsColdGas);
    update_from_star_formation_compat(merger_centralgal, stars, metallicity, galaxies, run_params);

    galaxies[merger_centralgal].BulgeMass += (1 - run_params->physics.RecycleFraction) * stars;
    galaxies[merger_centralgal].MetalsBulgeMass += metallicity * (1 - run_params->physics.RecycleFraction) * stars;

    // recompute the metallicity of the cold phase
    metallicity = get_metallicity(galaxies[merger_centralgal].ColdGas, galaxies[merger_centralgal].MetalsColdGas);

    // update from feedback
    update_from_feedback_compat(merger_centralgal, centralgal, reheated_mass, ejected_mass, metallicity, galaxies, run_params);

    // check for disk instability
    if(run_params->physics.DiskInstabilityOn && mode == 0) {
        if(mass_ratio < run_params->physics.ThreshMajorMerger) {
    ...
    if(galaxies[merger_centralgal].ColdGas > 1e-8 && mass_ratio < run_params->physics.ThreshMajorMerger) {
        const double FracZleaveDiskVal = run_params->physics.FracZleaveDisk * exp(-1.0 * galaxies[centralgal].Mvir / 30.0);  // Krumholz & Dekel 2011 Eq. 22
        galaxies[merger_centralgal].MetalsColdGas += run_params->physics.Yield * (1.0 - FracZleaveDiskVal) * stars;
        galaxies[centralgal].MetalsHotGas += run_params->physics.Yield * FracZleaveDiskVal * stars;
        // galaxies[centralgal].MetalsEjectedMass += run_params->Yield * FracZleaveDiskVal * stars;
    } else {
        galaxies[centralgal].MetalsHotGas += run_params->physics.Yield * stars;
