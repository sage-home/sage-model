 ...
 */
int gadget4_hdf5_initialize(const char *filename, struct params *params, void **format_data)
{
    if (filename == NULL || params == NULL || format_data == NULL) {
        LOG_ERROR("Invalid parameters passed to gadget4_hdf5_initialize");
        return -1;
    }

    // Allocate handler data structure
    struct gadget4_hdf5_data *data = mymalloc(sizeof(struct gadget4_hdf5_data));
    if (data == NULL) {
        LOG_ERROR("Failed to allocate memory for Gadget4 HDF5 handler data");
        return -1;
    }

    // Initialize the structure
    memset(data, 0, sizeof(struct gadget4_hdf5_data));
    ...
    LOG_DEBUG("Gadget4 HDF5 handler initialized for file: %s", filename);
 ...
 * @return 0 on success, non-zero on failure
 */
int gadget4_hdf5_cleanup(void *format_data)
{
    if (format_data == NULL) {
    ...
    LOG_DEBUG("Cleaning up Gadget4 HDF5 handler for file: %s", data->filename);
 ...
 * @return 0 on success, non-zero on failure
 */
int gadget4_hdf5_close_open_handles(void *format_data)
{
    if (format_data == NULL) {
        return 0;
    }
 ...
 */
int gadget4_hdf5_get_open_handle_count(void *format_data)
{
    if (format_data == NULL) {
        return 0;
    }
